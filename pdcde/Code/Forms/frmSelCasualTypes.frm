...
Begin VB.Form frmSelCasualTypes 
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "Search: casual types"
   ClientHeight    =   3810
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   6135
   ...
   End
End
Attribute VB_Name = "frmSelCasualTypes"
...
Set rsCheckMatch = CConnect.GetRecordSet("select * from EmpTerms where matchToCasual=1")
IIf rsCheckMatch.RecordCount > 0, frmSelCasualTypes.Tag = Trim(rsCheckMatch!Code & ""), frmSelCasualTypes.Tag = ""
...
'    strValue = CLng(lvwDetails.SelectedItem.Tag)
'    strDatePart = lvwDetails.SelectedItem.ListSubItems(1).Tag
'    'strID = lvwDetails.SelectedItem.ListSubItems(2).Tag
'    strID = Me.Tag
'    Unload Me
Exit Sub
...
Set rsCont = CConnect.GetRecordSet("select * from pdCasualTypes order by Code")
    Call InitGrid
    Call DisplayRecords
Set rsCont = Nothing
...
Set rsCont = New Recordset
    ...
    If OptCode.value = True Then
        Set rsCont = CConnect.GetRecordSet("select * from pdCasualTypes where  Code Like '" & X & "'" & " order by Code")
    Else
        Set rsCont = CConnect.GetRecordSet("select * from pdCasualTypes where  Description Like '" & X & "'" & " order by Description")
    End If
